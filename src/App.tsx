...
// --- Types ---
type TransactionType = 'ingreso' | 'egreso' | 'comision';
...
export default function App() {
  ...
  const totalsCaja = useMemo(() => {
    ...
    const egresos = filteredTransactions.filter(t => t.type === 'egreso' || t.type === 'comision').reduce((acc, t) => acc + t.amount, 0);
    ...
    autoTable(doc, {
      ...
      didParseCell: (data) => {
        if (data.section === 'body' && data.column.index === 2) {
          ...
          } else if (type === 'EGRESO' || type === 'COMISION') {
                        ...
                        <tr key={t.id} className="group hover:bg-white/5 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <span className={cn(
                              ...
                              t.type === 'ingreso' ? "bg-prime/10 text-prime" : 
                              t.type === 'comision' ? "bg-amber-500/10 text-amber-400" : "bg-rose-500/10 text-rose-400"
                            )}>
                              {t.type}
                            </span>
                          </td>
